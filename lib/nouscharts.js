/**
 * NousCharts v2 — Libreria de visualizacion de datos sobre Canvas API
 * 6 tipos de grafica, soporte dark mode via options de color.
 */
...
function drawLineChart(canvas, labels, values, options = {}) {
  ...
  const axisColor = options.axisColor || '#d9dde2';
  const labelColor = options.labelColor || '#6b7280';
  const lineColor = options.color || '#374151';
  const ptColor = options.pointColor || options.textColor || '#111827';
...
function drawAreaChart(canvas, labels, values, options = {}) {
  ...
  const { max } = getRange(values);
  const step = w / Math.max(values.length - 1, 1);
  const points = values.map((v, i) => ({ x: x + i * step, y: y + h - (v / max) * (h - 8) }));
  ...
  ctx.save();
  ctx.strokeStyle = options.stroke || options.color || '#374151';
  ctx.lineWidth = 2;
  ctx.beginPath();
  points.forEach((p, i) => { i === 0 ? ctx.moveTo(p.x, p.y) : ctx.lineTo(p.x, p.y); });
  ctx.stroke();
  ...
  labels.forEach((l, i) => text(ctx, l, x + i * step, y + h + 16, labelColor, 'center', 10));
}
...
function drawDonutChart(canvas, labels, values, options = {}) {
  ...
  const mutedColor = options.mutedColor || '#6b7280';
  const totalColor = options.textColor || '#111827';
  ...
  labels.forEach((label, i) => {
    ...
    text(ctx, `${label} (${values[i]})`, 28, ly, labelColor, 'left', 10);
  });
}
...
function drawRadarChart(canvas, labels, values, options = {}) {
  const ctx = canvas.getContext('2d');
  const bg = options.bg || '#ffffff';
  const gridColor = options.gridColor || '#eceff3';
  const axisColor = options.axisColor || '#eef0f3';
  const labelColor = options.labelColor || '#6b7280';
  const fillColor = options.fillColor || 'rgba(55,65,81,0.20)';
  const strokeColor = options.color || '#374151';
  ...
  labels.forEach((label, i) => {
    ...
    ctx.stroke();
    text(ctx, label, cx + Math.cos(ang) * (radius + 16), cy + Math.sin(ang) * (radius + 16), labelColor, 'center', 10);
  });
...
function drawHeatmap(canvas, rows, cols, matrix, options = {}) {
  ...
  matrix.forEach((line, r) => {
    line.forEach((value, c) => {
      ...
      text(ctx, value, padLeft + c * cw + cw / 2, padTop + r * ch + ch / 2 + 4, tc, 'center', 10, '600');
    });
  });

  rows.forEach((row, i) => text(ctx, row, padLeft - 8, padTop + i * ch + ch / 2 + 3, labelColor, 'right', 10));
  cols.forEach((col, i) => text(ctx, col, padLeft + i * cw + cw / 2, padTop + gridH + 16, labelColor, 'center', 10));
}
